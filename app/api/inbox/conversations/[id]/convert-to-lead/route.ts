...
export async function POST(
  ...
  try {
    ...
    // Reuse existing lead for this contact in this workspace if one already exists
    let lead = await prisma.lead.findFirst({
      where: { contactId: conversation.contactId, workspaceId: payload.workspaceId },
    });
    ...
    if (!lead) {
      ...
      lead = await createLead(payload.workspaceId, {
    ...
    // Link conversation to the lead
    await prisma.conversation.update({
      where: { id: conversationId },
      data: { leadId: lead.id },
    });

    return NextResponse.json({ lead }, { status: lead.createdAt === lead.updatedAt ? 201 : 200 });
  } catch (err) {
