...
import { moveLeadStage, logActivity } from "@/lib/domain/leads/service";
...
export async function POST(
  ...
  try {
    const body = await req.json() as { stageId: string };
    if (!body.stageId) return NextResponse.json({ error: "stageId required" }, { status: 400 });

    const lead = await moveLeadStage(id, body.stageId);
    ...
    await logActivity(
      ...
      id,
      "stage_change",
      `Lead moved to pipeline stage`,
      { stageId: body.stageId },
    );

    return NextResponse.json({ lead });
  } catch (err) {
    console.error(`[POST /api/leads/${id}/move]`, err);
